...
/**
 * Performance test for the AlertHistory entity.
 */
class AlertHistoryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AlertHistory entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all alertHistories")
            .get("/api/alert-histories")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created alertHistory")
                .get("${new_alertHistory_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created alertHistory")
            .delete("${new_alertHistory_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
