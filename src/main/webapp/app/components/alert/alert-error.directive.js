(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('stockadvisorApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-stockadvisorApp-error');
                var entityKey = httpResponse.headers('X-stockadvisorApp-params');
                if (errorHeader) {
